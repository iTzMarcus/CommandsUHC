...
public class ListCommand implements CommandExecutor {
	...
	public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
		...
		for (UUID player : alives) {
			...
			sender.sendMessage("§8⫸ " + PlayerUtils.getRank(player) + (TeamsUtils.getTeam(player) == null ? "§7" : TeamsUtils.getTeamPrefix(player)) + PlayerUtils.getName(PlayerUtils.getId(player)) + (online == null ? "" : " §8- §7" + ((online.getHealth() / 2) * 10) + "§4♥"));
